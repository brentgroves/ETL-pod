...
{
../misc/script-start.py 5
TIMEFORMAT='%R'; time ./AccountingPeriod.py '123681,300758,310507,306766,300757' "$username" "$password" "$username2" "$password2" 1>&4 2>&3
# TIMEFORMAT='%R'; time ./AccountingAccount.py '123681,300758,310507,306766,300757' 1>&8 2>&7
result=$?
if [[ $result -eq 0 ]]
then # if/then branch
  ../misc/script-end.py 5 0
else
  ../misc/script-end.py 5 1
fi
} 2>&6 
