...
{
../misc/script-start.py 116
TIMEFORMAT='%R'; time ./AccountPeriodBalanceDeletePeriodRange.py "$username2" "$password2" 1>&4 2>&3
# TIMEFORMAT='%R'; time ./AccountingAccount.py '123681,300758,310507,306766,300757' 1>&8 2>&7
result=$?
if [[ $result -eq 0 ]]
then # if/then branch
  ../misc/script-end.py 116 0
else
  ../misc/script-end.py 116 1
fi
} 2>&6 
...
if [[ $result -ne 0 ]]
then # if/then branch
  printf "AccountPeriodBalanceDeletePeriodRange script failed. \nerror message: $em \ndebug messages: $dm \ntime=$tm" | mail -s "MCP Script Failure" bgroves@buschegroup.com
fi
